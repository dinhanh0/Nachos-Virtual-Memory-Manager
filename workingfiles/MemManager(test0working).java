...
public class MemManager{
    ...
    private int queueTail;    // tail index for FIFO/ESC enqueue
    private int queueSize;    // number of valid items in queue
    ...
    MemManager( int pbuffer_in , int hbits_in )
    {
        ...
        else if( hbits < 0 )
        {
          ...
          // set up FIFO queue -- frames are enqueued when actually used
        ...
        // clear the counter for LRU (also used as fallback rr)
        counter = 0;
    }
    ...
    /////////////////////////////////////////////////////////////////////
    // Determine whether a target frame is in the page buffer. If in the 
    // buffer, return true. 
    ////////////////////////////////////////////////////////////////////
    ...
    // FIFO/ESC queue helpers
    private void enqueueFrame(int frame) {
        if (buffersize > 0 && InBuffer(frame)) return; // never queue buffer frames
    ...
    //////////////////////////////////////////////////////////////////////
    // Make a frame free using ESC, FIFO, or N-bit LRU replacement.
    // Return the number of the frame that will be paged out
    //////////////////////////////////////////////////////////////////////
    int makeFreeFrame() 
    {
      // Choose a victim frame based on replacement policy.
      // Never pick a buffer frame.
      if (policy == FIFO) {
          // Pop from head until we find a non-buffer frame.
          int tries = (queue == null) ? 0 : queue.length;
          while (tries-- > 0 && queueSize > 0) {
              int cand = dequeueFrame();
              if (!InBuffer(cand)) {
                  return cand;
              }
          }
          // Fallback: if queue is unexpectedly empty, use round-robin below.
      }

      // Default fallback (keeps existing behavior for other policies):
      // round-robin, skipping buffer frames.
      int victim = counter;
    ...
    void faultIn(TranslationEntry PTEntry) 
    {
      ...
      // When page buffering is disabled (test0), update FIFO queue when a frame
      // is allocated or reused.

      if( physPage >= 0 )
      { // A free frame is available from main memory.
        // enqueue for FIFO/ESC order now that this frame will hold a page
        ...
        // ? Re-enqueue reused frame so FIFO order is preserved after eviction
        if (policy == FIFO || policy == ESC) { enqueueFrame(physPage); }
    ...
    // MP3 :  Your code here (not needed for test0 / FIFO).
    /////////////////////////////////////////////////////
    void recordHistory( int arg )
    {
      // For LRU only: shift history and insert current USE bit, then clear USE.
      // Not required for FIFO test0.
    }
    ...
    void pageIn(TranslationEntry PTEntry, int physFrame)
    {
      ...
      // LRU history update - page has not been used yet
      if (policy == LRU)
      {
         history[physFrame] = 0;
      }
